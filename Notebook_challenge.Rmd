...
Analyse the propability of clients age
...
#QUESTION: IS THEIR A MINIMUM AND MAXIMUM AGE TO BE OWNER ?????????????????
...
#create variable duplicate accounts and print all of those
...
```
Results of the analysis: The clients seem to be partners because there is always a men and a women of the same age (+- 10 years)

Visualize those duplicate accounts
```{r}
#Showing the different account types according to clients age
...
```{r}
titanic %>% ggplot(aes(x = Pclass, y = Age, fill = Survived)) + geom_boxplot() + xlab('Passenger Class') + ylab('Age') + ggtitle(label = 'Boxplot Age-Passenger Class')
```

```{r}
titanic %>% ggplot(aes(x = Pclass, y = Age)) + geom_jitter(height = 0, alpha = 0.75, size = 1, width = 0.3, aes(color = Survived)) + facet_wrap( ~ Survived) + geom_boxplot(alpha = 0.5, aes(fill = Survived)) + xlab('Passenger Class') + ylab('Age') + ggtitle(label = 'Boxplot Surviving Chance')
```
